...
Four alcoves here are set with cheap cots where drunks can sleep off their hangovers. In the bow are four hammocks that serve a similar purpose. At any given time, `dice:1d4|noform|noparens|avg` (`1d4`) [unconscious](/3-Mechanics/CLI/conditions.md#Unconscious) [commoners](/3-Mechanics/CLI/bestiary/humanoid/commoner-xmm.md) can be found throughout this area, sleeping off the effects of a too-wild night. If the characters wish to rob them, each commoner has a belt pouch containing `dice:1d6|noform|noparens|avg` (`1d6`) sp and `dice:2d6|noform|noparens|avg` (`2d6`) cp.
