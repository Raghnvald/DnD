...
The wall can be destroyed, also one layer at a time, in order from red to violet, by means specific to each layer. Once a layer is destroyed, it remains so for the duration of the spell. [Antimagic field](3-Mechanics/CLI/spells/antimagic-field.md) has no effect on the wall, and [dispel magic](3-Mechanics/CLI/spells/dispel-magic.md) can affect only the violet layer.
