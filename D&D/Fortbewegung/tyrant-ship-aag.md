...
A spherical chamber on the ship's command deck functions as a [spelljamming helm](3-Mechanics/CLI/items/spelljamming-helm-aag.md) that only beholders can attune to. Any beholder that occupies this chamber can attune to it. Reducing a tyrant ship to 0 hit points not only destroys it but also destroys the ship's [spelljamming helm](3-Mechanics/CLI/items/spelljamming-helm-aag.md) and eyestalk cannons.

A tyrant ship can't float on water, but it can safely land on the ground.

```ad-statblock
title: Tyrant Ship
![](3-Mechanics/CLI/vehicles/token/tyrant-ship-aag.webp#token)
