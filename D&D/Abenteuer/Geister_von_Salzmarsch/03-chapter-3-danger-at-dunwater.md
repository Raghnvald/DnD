...
For every 10 minutes the characters spend traveling the corridors, roll a `dice:d12|noform|noparens|avg` (`d12`). On a 12, the characters come upon a guard patrol.
...
When triggered, the trap fills the entire room with noxious green gas. Creatures caught in the gas must succeed on a DC 10 Constitution saving throw or cough and choke loudly for `dice:1d4|noform|noparens|avg` (`1d4`) minutes; those affected by the gas are far more likely to attract the attention of wandering patrols and nearby inhabitants of the lair.
...
When the trap is triggered, a scything blade swings up, striking at any creature within 5 feet of the front of the chest. Affected creatures must make a DC 13 Dexterity saving throw, taking `dice:2d6|noform|noparens|avg|text(7)` (`2d6`) slashing damage on a failed save or half as much on a successful one.
...
When the trap is triggered, a small poisoned dart is ejected from the lock. Any creature standing in front of the lock is automatically struck by the needle, taking 1 piercing and `dice:1d4+2|noform|noparens|avg|text(4)` (`1d4 + 2`) poison damage.
...
When triggered, the trap fills the room with paralyzing gas. Creatures caught in the gas must succeed on a DC 10 Constitution saving throw or be paralyzed for `dice:1d4|noform|noparens|avg` (`1d4`) minutes. A creature can repeat the saving throw at the end of each minute.
