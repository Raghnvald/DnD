...
If the characters question Kevori (neutral good, human [scout](3-Mechanics/CLI/bestiary/humanoid/scout-xmm.md)), she shares the following information:
...
A [Detect Magic](3-Mechanics/CLI/spells/detect-magic-xphb.md) spell or similar effect reveals an aura of divination magic around the eyeball. Casting [Identify](3-Mechanics/CLI/spells/identify-xphb.md) while targeting the eyeball reveals its function and its key phrase. Alternatively, a character can rifle through Delvin's pockets and find a slip of parchment with the key phrase scribbled on it.
...
Characters who have a passive Wisdom ([Perception](3-Mechanics/CLI/rules/skills.md#Perception)) score of 12 or higher notice the entrance to a cramped tunnel hidden behind some collapsed rubble to the south. A character who spends 1 minute clearing the rubble opens the entrance to a crawlspace (area N7).
...
A neutral evil [water weird](3-Mechanics/CLI/bestiary/elemental/water-weird-xmm.md) lurks here. It tries to ambush and kill any creature that enters the tunnel.
...
Nestled among the bookshelves are five incense sticks (worth 10 gp each) and a [component pouch](3-Mechanics/CLI/items/component-pouch-xphb.md). A character who inspects the area and succeeds on a DC 15 Intelligence ([Investigation](3-Mechanics/CLI/rules/skills.md#Investigation)) check also finds a hidden compartment in one of the shelved books. Inside is a [Spell Scroll](3-Mechanics/CLI/items/spell-scroll-xdmg.md) of [Protection from Evil and Good](3-Mechanics/CLI/spells/protection-from-evil-and-good-xphb.md).
...
Most items stored in this tomb have rusted, but one shield remains intact. This shield is a cursed [Shield of Missile Attraction](3-Mechanics/CLI/items/shield-of-missile-attraction-xdmg.md); however, if the characters restore the ghost's identity, Chanelle Hallwinter gratefully gives the shield to the characters and removes the curse, transforming the shield into a [+1 Shield](3-Mechanics/CLI/items/1-shield-xdmg.md).
...
The three burbling masses are awakened piles of undead sludge, sloughed from the cult's twisted experiments (each uses the [gray ooze](3-Mechanics/CLI/bestiary/ooze/gray-ooze-xmm.md) stat block but is an Undead instead of Ooze). The masses ignore the room's difficult terrain and attack if approached.
...
Eight [zombies](3-Mechanics/CLI/bestiary/undead/zombie-xmm.md) shamble around this crypt. The zombies are hungry and immediately attack upon seeing a creature unless that creature succeeds on a DC 16 Dexterity ([Stealth](3-Mechanics/CLI/rules/skills.md#Stealth)) check.
...
Seven [cultists](3-Mechanics/CLI/bestiary/humanoid/cultist-xmm.md) sleep in these two chambers. Each cultist wears a hooded gray robe that has a desiccated eyeball (like the one obtained from Delvin) tucked in one pocket.
...
Inside the box are two [Potions of Healing (greater)](3-Mechanics/CLI/items/potion-of-greater-healing-xdmg.md).
...
Zalryr (neutral evil, human [cult fanatic](3-Mechanics/CLI/bestiary/humanoid/cultist-fanatic-xmm.md)) stands at the north end of the room, conducting an experiment on the two [cultists](3-Mechanics/CLI/bestiary/humanoid/cultist-xmm.md) standing inside the runic circle. Zalryr's experiment has magically siphoned the secrets from the cultists' souls to create the two shadowy entities (each uses the [shadow](3-Mechanics/CLI/bestiary/undead/shadow-xmm.md) stat block).
