...
Skills in a stat block are shown with the total modifier—the creature's ability modifier plus its proficiency bonus. If a creature's stat block says "Stealth +4," roll a `dice:d20|noform|noparens|avg` (`d20`) and add 4 when the creature makes an ability check using Stealth.
