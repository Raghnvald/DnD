---
Bezeichnung: Storm Cannon
linter-yaml-title-alias: Storm Cannon
tags:
  - Kompendium/Objekt/size/medium
  - Kompendium/Objekt/type/siege-weapon
  - Quelle/5e/hbtd
aliases:
  - "Storm Cannon"
  - Storm Cannon
---
# Storm Cannon
*Source: Hold Back The Dead p. 4*  

Aiming a Storm Cannon requires the Utilize action, then a creature can take the Lightning Ball action.

```ad-statblock
title: Storm Cannon
![](3-Mechanics/CLI/objects/token/storm-cannon-hbtd.webp#token)
*Medium Siege weapon*

- **Armor Class** 18
- **Hit Points** 30
- **Speed** 

|STR|DEX|CON|INT|WIS|CHA|
|:---:|:---:|:---:|:---:|:---:|:---:|
|10 (+0)|10 (+0)|10 (+0)|10 (+0)|10 (+0)|10 (+0)|


## Actions

***Lightning Ball (Requires Aim).*** *Ranged Attack Roll:* `+6`, range 600/800 ft. *Hit:* 22 (`4d10`) Lightning damage.
```
^statblock