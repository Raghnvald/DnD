...
While Cabal's Ruin is in a dormant state, the cloak has 4 charges and it regains `dice:1d4|noform|noparens|avg` (`1d4`) expended charges daily at dawn. When you hit with an attack, you can expend any number of charges from the cloak, with the attack dealing an extra `dice:1d6|noform|noparens|avg` (`1d6`) lightning damage per charge expended. If your attack strikes multiple targets, you choose one target that takes this extra damage.
